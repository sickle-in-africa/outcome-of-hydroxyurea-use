...
media_directory_path <- '/home/jack/computer/genemap/hydroxyurea-study/media/publication-ready-plots/light-theme/reasons-hu-use/'
...
study_data %>% group_by(using_hu) %>% summarise(count=n())
...
study_data_imputed %>% filter(currently_using_hu==TRUE) %>% nrow -> n_using_hu
study_data_imputed %>% filter(currently_using_hu==FALSE) %>% nrow -> n_stopped_hu
study_data_imputed %>%
update_reasons_columns() %>%
    pivot_longer(cols=c(
        hu_is_affordable,
