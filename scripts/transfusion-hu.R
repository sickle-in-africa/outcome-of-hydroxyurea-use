...
currently_using_hu %>% mutate(transfused_before=as.factor(transfused_before))  %>% ggplot(aes(x=transfused_before, y=current_hu_dose, fill=transfused_before)) + geom_violin()
ggsave('./media/transfused_before-hudose-violin.png',dpi=300)

currently_using_hu %>% mutate(transfused_before=as.factor(gender))  %>% ggplot(aes(x=gender, y=current_hu_dose, fill=gender)) + geom_violin()
ggsave('./media/gender-hudose-violin.png',dpi=300)
