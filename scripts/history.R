source('./scripts/compare-doses.R')
ls()
...
studyData
studyData %>% filter(using_hu==1)
studyData %>% filter(using_hu==1) %>% ggplot + geom_point(aes(x=start_hu_dose,y=current_hu_dose)) + geom_abline(intercept = 0, slope = 1)
ls()
rm(currently_using_hu, registry, studyData, survey)
...
currently_using_hu %>% ggplot + geom_point(aes(x=start_hu_dose,y=current_hu_dose)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% ggplot + geom_point(aes(x=start_hu_dose,y=current_hu_dose,color=age)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% ggplot + geom_point(aes(x=start_hu_dose,y=current_hu_dose,color=age)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=age)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=freq_hu_use)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% mutate(freq_hu_use = as.factor(freq_hu_use)) %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=freq_hu_use)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% mutate(freq_hu_use = as.factor(freq_hu_use)) %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=freq_hu_use)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% mutate(freq_hu_use = as.factor(freq_hu_use)) %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=hu_not_expensive)) + geom_abline(intercept = 0, slope = 1)
...
currently_using_hu %>% mutate(available_pharm = as.factor(available_pharm)) %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=available_pharm)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% mutate(easy_to_take = as.factor(easy_to_take)) %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=easy_to_take)) + geom_abline(intercept = 0, slope = 1)
...
currently_using_hu %>% mutate(freq_pain_after_hu = as.factor(freq_pain_after_hu)) %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=freq_pain_after_hu)) + geom_abline(intercept = 0, slope = 1)
currently_using_hu %>% ggplot + geom_jitter(aes(x=start_hu_dose,y=current_hu_dose,color=freq_pain_after_hu)) + geom_abline(intercept = 0, slope = 1)
savehistory('./scripts/history.R')
